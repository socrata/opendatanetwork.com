...
// The first part of this file is the data sources scripts.  Need to copy the latest over the top if we are 
// regenerating.  Go to line 1450 or so below to where the sitemap generation begins.
//
...
// Sitemap generation begins here
//
const fs = require('fs');
const readline = require('readline');
const request = require('request');

const outputDatasetsOK = __dirname + '/output/urls-datasets-ok.txt';
const outputDatasetsToCheck = __dirname + '/output/urls-datasets-classroom-statistics-to-check.txt';
const maxUrls = 60000;
var urlCount = 0;

// Create the output file
//
const outputStreamDatasetsOK = fs.createWriteStream(outputDatasetsOK);
const outputStreamDatasetsToCheck = fs.createWriteStream(outputDatasetsToCheck);
...
function generateRegionUrls(successHandler) {
    ...
    rd.on('line', line => {
        ...
        DATASETS.forEach(dataset => {
            ...
            if (dataset.regions.indexOf(regionType) == -1)
                return;

            if ((typeof dataset.include != 'undefined') && !dataset.include(region))
                return;

            if (dataset.vector == 'classroom_statistics')
                generateUrl(outputStreamDatasetsToCheck, url + '/' + dataset.vector, population, regionId);
            else
                generateUrl(outputStreamDatasetsOK, url + '/' + dataset.vector, population);
...
function generateUrl(stream, url, population, regionId) {

    stream.write(url);
    stream.write('\t');
    
    if (regionId) {
        stream.write(regionId);
        stream.write('\t');
    }
